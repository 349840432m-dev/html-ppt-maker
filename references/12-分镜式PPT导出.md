...
- 通过 `scripts/run_tool.sh` 运行 `expand_storyboard_plan.py deck-plan.json storyboard-plan.json`。
- 检查输出页数不少于原页数。
- 检查每个分镜 id 唯一。
- 检查 `source_slide_id` 可以追溯回原始页面。
- 运行 `export_html_matched_pptx.mjs` 时同时传入原始 `deck-plan.json` 和 `storyboard-plan.json`。
- 运行 `verify_html_matched_export.py <导出清单>`；它会检查每个状态的 `visible_steps`、截图尺寸与哈希、`source_slide_id`，并阻止同一源页的不同状态导出成完全相同的画面。
