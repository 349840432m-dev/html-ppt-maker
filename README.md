...
- **制作前风格确认**：先从 30 类语义骨架中选择内容表达，再在同一页面上比较 7 个实时视觉主题。
...
图库只保留一套 30 页语义骨架：12 类基础骨架负责页面角色，18 类表达骨架负责条件、因果、转化、层级、改造、诊断、系统与范围。7 个视觉主题作用于同一份页面 DOM，不复制内容，也不改变节点、文字和阅读顺序。
...
打开 [`assets/style-library/index.html`](assets/style-library/index.html) 可以按 LAYOUT 选择内容关系，再用 THEME 比较同一页面在不同视觉语言下的表现。

正式图库始终只有 30 个页面。`30 × 7 = 210` 仅表示可选的发布前全矩阵渲染状态，不是 210 个独立模板，也不是日常修改的固定回归成本。
...
python3 ~/.codex/skills/html-ppt-maker/scripts/audit_style_library.py \
  ~/.codex/skills/html-ppt-maker
```

图库浏览器回归默认检查 30 个唯一页面、7 次主题换肤、DOM 身份、桌面与 390px 窄屏溢出：

```bash
node ~/.codex/skills/html-ppt-maker/scripts/render_style_gallery.mjs \
  ~/.codex/skills/html-ppt-maker/assets/style-library/index.html \
  ~/.codex/skills/html-ppt-maker/assets/style-library/style-gallery.png
```

只有发布前需要完整主题 × 版式组合时，才额外传入 `--full-matrix`。
...
├── assets/
│   ├── style-library/             # 30 个唯一语义页面与 7 个实时主题
